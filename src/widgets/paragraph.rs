...
use crate::widgets::scroll::{OffsetScroller, TailScroller, Scroller, ScrolledLine};
    ...
    T: Iterator<Item = &'t Text<'t>>,
{
    fn draw(&mut self, area: Rect, buf: &mut Buffer) {
        ...
        let line_composer: Box<dyn LineComposer<'t>> = if self.wrapping {
        ...
        let mut scrolled_lines: Box<dyn Scroller<'t>> = match self.scroll_mode {
            ...
            ScrollMode::Tail => {
                let scroller = TailScroller::new(self.scroll, line_composer, text_area.height);
                Box::new(scroller)
            }
        };
